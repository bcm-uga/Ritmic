...
- Matrix T of reference cell lines: proportion matrix ge x nc
  - ge: gene expression
...
matrix_A <- readRDS("~/Datas/projects/data/extdata/A_60.rds")
dim(matrix_A)

# Matrix T control 
matrix_T_control <- readRDS("~/Datas/projects/data/extdata/T_60_tum_rna_c_t.rds")

pre_treat_res <- RiTMIC::pre_treat(penda_res = test_matrix, patientNumber=5)

```

```{r,label="RiTMIC::calc_dist"}
distance <- RiTMIC ::calc_dist(pre_treat_output = pre_treat_res, matrix_A = matrix_A)
calc_corr <- RiTMIC::calc_corr(matrix_T = matrix_T_control, matrix_A = A_60p)
```

```{r,label="RiTMIC::plot_res"}
RiTMIC::plot_res(calc_corr_output = calc_corr, calc_dist_output = distance, graph_title = "Analyse the model improvement thanks to PenDA with ROC curves")
```
