...
## Data filtering
```{r echo=FALSE}
load("~/Datas/projects/RiTMIC/data/matrix_D.RData")
load("~/Datas/projects/RiTMIC/data/matrix_D2.RData")
penda_data_case <- penda::penda_data_case

matrix_D_control <- matrix_D[,41]

rownames(matrix_D_data) <- rownames(penda_data_case)
colnames(matrix_D_data) <-colnames(penda_data_case)
...
data_bypatient2 = generate_data_bypatient(D_list = penda_res2$down_genes,
                                         U_list = penda_res2$up_genes)

plot_figure(data_patients = data_bypatient)
plot_figure(data_patients = data_bypatient2)
```

```{r fig.width=9, fig.height=9, echo=FALSE}
plot_heatmap_hclust(data = penda_res$down_genes - penda_res$up_genes)
plot_heatmap_hclust(data = penda_res2$down_genes - penda_res2$up_genes)
```
