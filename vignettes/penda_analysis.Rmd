...
matrix_D = readRDS("~/Datas/projects/RiTMIC/inst/extdata/matrix_D.rds")
matrix_D2 = readRDS("~/Datas/projects/RiTMIC/inst/extdata/matrix_D2.rds")
penda_data_case <- penda::penda_data_case

matrix_D_control <- penda::penda_data_ctrl[,4]

Penda_dataset2 = penda::make_dataset_1ctrl(matrix_D_control, matrix_D2)
data_ctrl2 = Penda_dataset2$data_ctrl
data_case2 = Penda_dataset2$data_case
...
L_H_list2 = penda::compute_lower_and_higher_lists_1ctrl(data_ctrl2, s_max = s_max)
L2 = L_H_list2$L
H2 = L_H_list2$H
...
penda_res2 = penda::penda_test_1ctrl(samples = data_case2, 
                   iterations =  iterations, 
                   L_H_list =  L_H_list2, 
                   threshold = threshold)
```
...
data_bypatient2 = RiTMIC::generate_data_bypatient(D_list = penda_res2$down_genes,
                                         U_list = penda_res2$up_genes)

RiTMIC::plot_figure(data_patients = data_bypatient)
RiTMIC::plot_figure(data_patients = data_bypatient2)
```

```{r fig.width=9, fig.height=9, echo=FALSE}
RiTMIC::plot_heatmap_hclust(data = penda_res$down_genes - penda_res$up_genes)
RiTMIC::plot_heatmap_hclust(data = penda_res2$down_genes - penda_res2$up_genes)
```
