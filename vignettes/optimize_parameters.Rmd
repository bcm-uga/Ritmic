...
assign(paste('plot_',threshold,sep=''),plot)
}

library("cowplot")
plotlist <- paste('plot_',listf,sep="")
plotlist
plot_grid(plot_0.1, plot_0.6, plot_0.8,plot_0.86, plot_0.92,plot_0.98,
          ncol = 2, nrow = 3)
df <- data.frame(df_0.1,"threshold"=0.1)

df_0.2['threshold'] <- threshold
list2 <- listf[2:28]
...
options(warn = -1)
cor_T60_c = c()
pb <- progress_bar$new(format = "  Running RiTMIC::pre_plot_res [:bar] :current/:total (:percent) in :elapsed",total = nrow(D_2v2_c), clear = FALSE, width= 80)
for(g in 1:nrow(D_2v2_c)){
  pb$tick()
  for(t in 1:nrow(EDec_80p_a)){
    c = cor(D_2v2_c[g, ], EDec_80p_a[t, ])
    cor_T60_c = rbind(cor_T60_c, c(g, t, c))
  }
}
options(warn = 0)
saveRDS(cor_T60_c,file = '~/Datas/projects/data/optimize_parameters/cor_T60_c')
```