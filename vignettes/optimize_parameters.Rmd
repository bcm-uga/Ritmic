...
for (threshold in listf){
  ...
  # Initialization
  if (threshold == 0.10) {
    dfFinal = data.frame(pval = as.factor(rep(pvalues)),
    ...
    # Continue
    temp = data.frame(pval = as.factor(rep(pvalues)),
                  FPR = c(res_ks[5, ], res_st[5, ], res_kanto[5, ], res_cor[5, ]),
                  TPR = c(res_ks[6, ], res_st[6, ], res_kanto[6, ], res_cor[6, ]),
                  "threshold" = threshold,
                  metrique = rep(c("ks", "student", "kanto", "correlation"),each = length(pvalues)))
...
saveRDS(dfFinal, file = '~/Datas/projects/data/optimize_parameters/df.RDS')
```
```{r,echo=TRUE}
df <- readRDS('~/Datas/projects/data/optimize_parameters/df.RDS')
