...
For this step, we use D_rna, the transcriptome of each bulk samples. We also use the A matrix with the proportion of each cell type in each sample, and the T matrix with the cell type profiles. A and T matrices was obtain with the vignette 1_deconvolution.Rmd.
...
The first part of this step is to identify the cancer cell type. For that, we can used different methods like cell markers, comparison with identified cell lines, etc. Here, to automate the process and as we are in the simulation framework, we compared the obtained A matrix with the reference A matrix.  

We know that the real tumor is the 4 of A ref, so we pick the cell type of A_calc with the biggest correlation. 
