...
pre_treat_res <- Ritmic::pre_treat(penda_res = test_matrix, thres_p = 5)

T_cancer = readRDS("~/Datas/projects/RiTMIC/inst/extdata/matrix_T.RDS")
```

```{r,label="RiTMIC::calc_dist", message=FALSE}
distance <- Ritmic::calc_dist(binary_penda = pre_treat_res, A = matrix_A)
correlation <- Ritmic::calc_corr(D_cancer = simu_D_output, A = matrix_A)
```
...
corr_3 = correlation[correlation$type == 3, ]
dist_3 = distance[distance$type == 3, ]

Ritmic::plot_res(corr_res = correlation, dist_res = distance, genes_dereg = T_cancer$genes_dereg, pvalues, graph_title = "ROC curves to test the model improvement induce by PenDA")
```
